...
ディレクトリ内のPNG/JPG画像の順番をD&Dで自由に変更し、確定時に連番（010, 020, 030...）でリネームするWindowsネイティブアプリ。手動で間に画像を挿入できるよう、10刻みの連番を採用する。
...
| `setup.bat` | 初回セットアップ用バッチファイル（venv作成・依存インストール） |
...
- 選択ディレクトリ内の `.png` / `.jpg` / `.jpeg` ファイルをファイル名昇順でロード
- 大文字小文字を区別しない（`.PNG` `.JPG` `.JPEG` も対象）
- JPEGのEXIF Orientationタグに従って画像を回転補正してから読み込む（サムネイル・拡大表示とも）
...
- リネーム形式: 10刻みの連番（010.png, 020.png, 030.png, ...）
  - 拡張子は元のファイルのもの（png/jpg/jpeg）を維持し、常に小文字化する（`.PNG`→`.png`）
- ゼロパディング桁数: 最大番号が1000未満なら3桁、以上なら桁数に合わせる
- 衝突回避: 2フェーズ方式
  1. 全ファイルを一時名（`__temp_NNNNNN{元の拡張子（小文字化）}`）にリネーム
  2. 一時名から最終連番名にリネーム
- リネーム後、表示を更新
...
- ウィンドウ下部に「削除」ボタンを表示（Canvas下のバー、高さ `VIEWER_BAR_HEIGHT`）
  - 押すと表示中の画像をごみ箱に即座に移動（`send2trash` 使用、確認ダイアログなし。右クリック削除と同じ挙動）
  - 削除後は次の画像（末尾なら1つ前）を続けて表示。0枚になった場合はビューアを閉じる
  - 背景のグリッドも同時に更新される
...
### 12. 右クリックメニュー（コンテキストメニュー）

- サムネイルを右クリックするとコンテキストメニューを表示
- 「削除」: 対象ファイルをごみ箱に即座に移動（`send2trash` 使用、確認ダイアログなし）
  - 削除後はリストから除外し、グリッドを再描画
  - 画像が0枚になった場合は「リネーム実行」ボタンを無効化
- サムネイル外の領域を右クリックした場合はメニューを表示しない
...
- トップバーの「生成日順(新しい方が下)」ボタンで、現在の画像リストをファイル更新日時（`os.path.getmtime`）の昇順に並べ替える
  - `getctime`（作成日時）はOneDriveなどの同期フォルダで再同期のたびに現在時刻へリセットされ信頼できないため使用しない
- 新しい方が下（末尾）になるよう昇順ソート
- ソート後はグリッドを再描画。ドラッグ操作等で任意の順序に変更した後も再度実行可能
- 画像読み込み中、または画像が0枚の場合はボタンを押しても何もしない
...
| `VIEWER_BAR_HEIGHT` | 40 | ビューアの削除ボタンバーの高さ（px） |
